...
any;
    ...
    const meshes = renderer.scene.findAll((n:
any) => isInstancedMesh(n.instance));
    // One for particles, one for rockets
    expect(meshes.length).toBe(2);

    const particleMesh = meshes.find((m:
any) => (m.instance as InstancedMesh).count === 8000);
...
any) => isInstancedMesh(n.instance))
      .map((node:
any) => node.instance as InstancedMesh)
      .find((mesh:
any) => mesh.count !== 8000);
